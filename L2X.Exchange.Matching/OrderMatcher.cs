...
using L2X.Exchange.Matching.Providers;
...
public class OrderMatcher(ITradeHandler? tradeHandler = null, IFeeProvider? feeProvider = null, decimal stepSize = 0.00000001m, int pricePrecision = 8)
{
    ...
    private readonly int _pricePrecision = pricePrecision >= 0 ? pricePrecision : throw new MatchingException("currencyPrecision");

    private readonly decimal _stepSize = stepSize >= 0 ? stepSize : throw new MatchingException("stepSize");

    private readonly OrderBook _book = new();

    private readonly OrderIdTracker _trackers = new();

    private readonly Queue<IReadOnlyList<IPriceLevel<Order>>> _stopOrderQueue = new();
    ...
    public IOrderBook<Order> Book => _book;

    public decimal MarketPrice => _marketPrice;

    public IEnumerable<Order> AllOrders => _book.Orders;
    ...
    private async Task<bool> AddTip(Order order)
    {
    ...
    {
        if (!_book.TryGetOrder(orderId, out Order? order) || order == null) return MatchState.Order_Not_Exists;
    ...
    private async Task CancelIncomingOrder(Order order)
    {
    ...
    private async Task<bool> CancelByCondition(Order order, long timeStamp, bool canBeFilled)
    {
    ...
    private async Task DecrementVolume(Order order, decimal decrement, OrderBook? orderBook)
    {
        if (orderBook != null)
    ...
    private async Task<bool> SelfMatchDecrement(Order newOrder, Order matchOrder)
    {
    ...
    private async Task MatchAndAddOrder(Order order, OrderCondition? condition = null)
    {
    ...
    private async Task MatchWithOpenOrders(Order order)
    {
        while (true)
        {
            Order? matchOrder = _book.GetBestOrderToMatch(!order.Side);
    ...
    private MatchState ValidateOrder(Order order)
    {
    ...
    public async Task<MatchState> AddOrder(Order order, long timeStamp, bool isOrderTriggered = false)
    {
