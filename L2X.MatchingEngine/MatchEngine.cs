using L2X.MatchingEngine.Collections;
...
public class MatchEngine(ITradeHandler? tradeHandler = null, IFeeProvider? feeProvider = null, decimal stepSize = 0.00000001m, int pricePrecision = 8)
{
    ...
    public IEnumerable<long> AcceptedOrders => _trackers;

    public IEnumerable<KeyValuePair<long, HashSet<long>>> GoodTillDates => _book.GoodTillDates;
    #endregion
    ...
    private async Task<MatchState> CancelOrder(long orderId, CancelReason cancelReason)
    {
    ...
    private async Task MatchAndAddOrder(MOrder order, OrderCondition? condition = null)
    {
		...
		var plc = _book.GetPriceLevel(order.Side, order.Price ?? 0);
        await _tradeHandler.OnOrderBookChange(order.Side, plc?.Price ?? order.Price ?? 0, plc?.Volume ?? 0);
	}
    ...
    public async Task<MatchState> CancelOrder(long orderId)
