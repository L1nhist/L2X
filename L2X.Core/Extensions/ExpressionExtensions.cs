...
public static class ExpressionExtensions
{
    ...
    public static IEnumerable<PropertyInfo> GetProps(this object obj, params string[] fields)
    {
        try
        {
            ...
            return props.Where(p => names.Any(n => p.Name.Equals(n, StringComparison.OrdinalIgnoreCase) || $"-{p.Name}".Equals(n, StringComparison.OrdinalIgnoreCase))).ToArray();
        }
        catch { }
    ...
    public static string GetPropName<TOwn, TFld>(this Expression<Func<TOwn, TFld>> expession)
    {
        ...
        if (expession.Body is MemberExpression mexp)
        {
            member = mexp;
        }
        else if (expession.Body is UnaryExpression uexp)
        {
            member = (MemberExpression)uexp.Operand;
        }
        else
        {
