...
{
    #region Operators
    public static implicit operator Expression<Func<T, bool>>([NotNull] FilterExpression<T> filter)
        => filter._exp ?? TrueExp;

    public static implicit operator Func<T, bool>([NotNull] FilterExpression<T> filter)
        => filter.Compile();

    public static implicit operator FilterExpression<T>([NotNull] Expression<Func<T, bool>> expession)
        => new(expession);
    #endregion
    ...
    public static FilterExpression<T> New(Expression<Func<T, bool>>? expession = null)
        => new(expession);

    public static FilterExpression<T> New(IEnumerable<T> enumerable, Expression<Func<T, bool>>? expession = null)
        => new(expession);
    ...
    internal FilterExpression(Expression<Func<T, bool>>? expession = null)
        => _exp = expession;
